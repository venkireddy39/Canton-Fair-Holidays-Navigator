    ...
    <>
      ...
      <article style={{ padding: '140px 20px 60px', background: 'var(--bg-dark)', minHeight: '80vh' }}>
        <div className="container" style={{ maxWidth: '800px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '20px', marginBottom: '25px', fontSize: '0.95rem', color: 'var(--text-muted)' }}>
          ...
          <div style={{ 
            ...
            <h3 style={{ fontSize: '1.5rem', color: 'var(--text-light)', marginBottom: '15px', fontFamily: 'var(--font-serif)' }}>
              Plan Your Canton Fair 2026 Sourcing Trip
            </h3>
            <p style={{ marginBottom: '20px', color: 'var(--text-muted)', fontSize: '1.05rem' }}>
      ...
      {relatedPosts.length > 0 && (
        <section className="page-section" style={{ background: 'rgba(255, 255, 255, 0.4)', borderTop: '1px solid var(--border-color)', paddingTop: '60px', paddingBottom: '80px' }}>
          <div className="container" style={{ maxWidth: '1200px', margin: '0 auto' }}>
            <h2 style={{ fontSize: '2.2rem', fontWeight: '800', color: 'var(--text-light)', marginBottom: '40px', textAlign: 'left', fontFamily: 'var(--font-serif)' }}>
              ...
              {relatedPosts.map(rp => (
                <Link key={rp.id} to={`/blog/${rp.slug}`} style={{ textDecoration: 'none' }}>
                  <div className="glass-card" style={{ padding: '30px', borderRadius: '16px', border: '1px solid var(--border-color)', height: '100%', transition: 'all 0.3s ease', display: 'flex', flexDirection: 'column' }} onMouseOver={e => { e.currentTarget.style.transform = 'translateY(-5px)'; e.currentTarget.style.borderColor = 'var(--accent-green)'; }} onMouseOut={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.borderColor = 'var(--border-color)'; }}>
                    <div style={{ color: 'var(--accent-gold)', fontSize: '0.85rem', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '12px' }}>{rp.category}</div>
                    <h3 style={{ fontSize: '1.35rem', color: 'var(--text-light)', marginBottom: '12px', lineHeight: '1.4', fontFamily: 'var(--font-serif)' }}>{rp.title}</h3>
                    <p style={{ color: 'var(--text-muted)', fontSize: '0.95rem', lineHeight: '1.6', flexGrow: 1 }}>{rp.excerpt.substring(0, 120)}...</p>
                    <span style={{ color: 'var(--accent-green)', fontWeight: '700', fontSize: '0.95rem', display: 'inline-flex', alignItems: 'center', gap: '5px', marginTop: '20px' }}>Read More →</span>
                  </div>
                </Link>
              ))}
            </div>
